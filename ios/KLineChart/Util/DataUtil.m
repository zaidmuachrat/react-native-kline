...
+(void)calcMACD:(NSArray<KLineModel *> *)dataList isLast:(BOOL)isLast {
    ...
    for (; i < dataList.count; i++) {
        ...
        // Check for NaN or infinity and handle it
        if (!isfinite(dif)) dif = 0;
        if (!isfinite(dea)) dea = 0;
        if (!isfinite(macd)) macd = 0;
...
+(void)calcVolumeMA:(NSArray<KLineModel *> *)dataList isLast:(BOOL)isLast {
    ...
    for (; i < dataList.count; i++) {
        ...
        // Only accumulate and calculate if volume is non-zero
        if (entry.vol > 0) {
            volumeMa5 += entry.vol;
            volumeMa10 += entry.vol;
        }
        ...
        if (i >= 4) {
            if (i == 4) {
                ...
                // Subtract the volume from 5 periods ago only if it's non-zero
                if (dataList[i - 5].vol > 0) {
                    volumeMa5 -= dataList[i - 5].vol;
                }
                entry.MA5Volume = volumeMa5 / 5;
            }
            if (!isfinite(entry.MA5Volume)) entry.MA5Volume = 0;
        } else {
            entry.MA5Volume = 0;
        }
        ...
        if (i >= 9) {
            if (i == 9) {
                ...
                // Subtract the volume from 10 periods ago only if it's non-zero
                if (dataList[i - 10].vol > 0) {
                    volumeMa10 -= dataList[i - 10].vol;
                }
                entry.MA10Volume = volumeMa10 / 10;
            }
            if (!isfinite(entry.MA10Volume)) entry.MA10Volume = 0;
        } else {
            entry.MA10Volume = 0;
        }
    }
...
+(void)calcKDJ:(NSArray<KLineModel *> *)dataList isLast:(BOOL)isLast {
    ...
    for (; i < dataList.count; i++) {
        ...
        // Check for NaN or infinity and handle it
        if (!isfinite(entity.k)) entity.k = 0;
        if (!isfinite(entity.d)) entity.d = 0;
        if (!isfinite(entity.j)) entity.j = 0;
    }
}
...
+(void)calcWR:(NSArray<KLineModel *> *)dataList isLast:(BOOL)isLast {
    ...
    for (; i < dataList.count; i++) {
        ...
        // Check for NaN or infinity and handle it
        if (!isfinite(entity.r)) entity.r = 0;
    }
}
